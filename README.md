...
当前已经实现服务器、Windows/macOS 共用桌面客户端和 Docker 部署配置。智能资料库支持点击批量选择，也支持从 Finder 或资源管理器直接拖入多个文件或整个文件夹；文件夹会递归扫描并以三路并发上传，随后自动去重、分类和索引。MVP 支持文字剪贴板；图片与富文本剪贴板将在后续版本加入。
...
连接令牌只在主进程中使用；设置页只显示“令牌已保存”，留空保存不会清除原令牌。若开机阶段暂时未读到配置，客户端会每 5 秒重新读取本地配置并自动连接。

智能资料库的拖放区既接受文件，也接受整个文件夹；文件夹中的子目录会递归扫描并上传。单次最多处理 10,000 个文件，更大的目录建议交给“备份中心”做版本化备份。
